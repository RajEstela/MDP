...
def _draw_legend(surface, font, font_b, routes, phase):
    n_perms = math.factorial(5)
    x, y = 8, 8
    surface.blit(
        font_b.render(f"Top {len(routes)} of {n_perms} routes:", True, (210, 210, 210)),
        (x, y),
    )
    y += 24
...
def _draw_hud(surface, font, font_b, elapsed_s, opt_len, visited, paused, done):
    bar = pygame.Surface((ARENA_PX, 36), pygame.SRCALPHA)
    bar.fill((0, 0, 0, 170))
    surface.blit(bar, (0, ARENA_PX - 36))
    if done:
        status, scol = "All images recognized!", (0, 255, 120)
    elif paused:
    ...
    mid   = font_b.render(f"Optimal: {opt_len:.0f} cm  |  Visited: {visited}/5", True, (255, 215, 0))
...
def _compute(screen, font_b, obstacles):
    ...
    msg = font_b.render(
        f"Computing optimal path across {math.factorial(len(obstacles))} permutations…",
        True, (255, 215, 0),
    )
...
def main() -> None:
    ...
    pygame.display.set_caption("MDP Simulator — Optimal Hamiltonian Path")
    clock = pygame.time.Clock()
    font   = pygame.font.SysFont('Arial', 16)
    font_b = pygame.font.SysFont('Arial', 18, bold=True)

    def new_run():
        obs = generate_random_obstacles()
    ...
    while running:
        ...
        if phase == 'show_all':
            ...
            draw_obstacles(screen, obstacles)
            _draw_legend(screen, font, font_b, routes, phase)
            ...
            _draw_hud(screen, font, font_b, anim_frames / FPS, opt_len,
                      obstacles_visited, paused, phase == 'done')
